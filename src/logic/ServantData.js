...
//   ENEMY: [/single enemy/i, /Deal significant damage/i],
//   ENEMIES: [/all enemies/i],
// };
...
//   {
//     role: "AOE_DPS",
//     when: ({ effects, targets }) => targets.includes("ENEMIES"),
//   },
//   {
//     role: "ST_DPS",
//     when: ({ effects, targets }) => targets.includes("ENEMY"),
//   },
//   {
//     role: "Support",
...
// // =========================================
// export function skillDataExtract(npText) {
//   const skills = normalize(npText);
...
// // =========================================
// function scoreNP(npText) {
//   const data = skillDataExtract(npText);
...
//   const npTexts = servant.noblePhantasms?.map((np) => np.effectFlags) ?? [];
...
//   npTexts.forEach((text) => {
//     const score = scoreNP(text);
...
export function assignRole(effectFlags = []) {
  const roles = [];

  if (effectFlags.includes("attackEnemyAll")) {
    roles.push("AOE DPS");
  }

  if (effectFlags.includes("attackEnemyOne")) {
    roles.push("ST DPS");
  }
  if (effectFlags.includes("support")) {
    roles.push("Support");
  }
